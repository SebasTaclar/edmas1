   ...
   <form @submit.prevent="handleSubmit" class="modal-form">
    <div class="form-body">
     ...
     <div class="form-group">
      ...
      <input id="category-name" v-model="formData.name" type="text" class="form-input" :class="{ 'error': errors.name }"
       placeholder="Ej: Adultos Masculino, Juvenil, etc." autocomplete="off" />
     ...
     <div class="form-group">
      ...
      <textarea id="category-description" v-model="formData.description" class="form-textarea"
       :class="{ 'error': errors.description }" placeholder="Describe brevemente esta categoría..." rows="3"></textarea>
...
import categoryService, { type CreateCategoryRequest } from '@/utils/categoryService'
...
const validateName = () => {
 ...
 // Verificar que no exista otra categoría con el mismo nombre (solo en creación)
 if (!isEditing.value) {
  ...
  // En edición, verificar que no exista otra categoría con el mismo nombre
  const categories = categoryService.getAllCategories()
  const existingCategory = categories.find(cat =>
  ...
  if (existingCategory) {
   errors.value.name = 'Ya existe otra categoría con este nombre'
   return false
  }
 }
...
const getTeamsCount = (): number => {
 ...
 return teams.filter(team => team.category === props.category!.id).length
}
...
const handleSubmit = async () => {
 ...
 try {
  ...
  if (isEditing.value && props.category) {
   ...
   result = categoryService.updateCategory(props.category.id, formData.value)
  } else {
   // Crear nueva categoría
   result = categoryService.createCategory(formData.value)
  }

  if (result.success) {
   emit('save')
  } else {
   generalError.value = result.error || 'Error al guardar la categoría'
  }
 } catch (error) {
  generalError.value = 'Error inesperado al guardar la categoría'
 } finally {
