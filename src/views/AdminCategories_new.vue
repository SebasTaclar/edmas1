<template>
 <main class="admin-main">
      ...
      <tbody>
       <tr v-for="category in filteredCategories" :key="category.id" class="table-row">
        ...
        <td class="actions">
         ...
         <button @click="confirmDelete(category)" class="btn-action delete" :disabled="isCategoryInUse(category.id)"
  ...
  <ConfirmationModal v-if="showDeleteModal" :title="'Eliminar Categoría'"
   :message="`¿Estás seguro de que deseas eliminar la categoría '${categoryToDelete?.name}'?`" :confirmText="'Eliminar'"
...
const getTeamsCount = (categoryId: string): number => {
 const teams = teamService.getAllTeams()
 return teams.filter(team => team.category === categoryId).length
}

const isCategoryInUse = (categoryId: string): boolean => {
 return categoryService.isCategoryInUse(categoryId)
}
...
const handleDelete = () => {
 if (categoryToDelete.value) {
  const success = categoryService.deleteCategory(categoryToDelete.value.id)
  if (success) {
   loadCategories()
  }
 }
